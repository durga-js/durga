...
class DurgaServer {
	...
	/**
	 * Simulate connection event (for testing purpose only!)
	 * @param  {Object} conf { event }
	 * @return {Promise}      resolves with { connection, result, listen }
	 */
	inject(conf, listener) {
		let connection = conf.connection || this.createConnection();
		...
		return new Promise(resolve => {
			...
			resolve(connection.dispatch(conf.event).then(result => {

				return {
					connection,
					result,
					listen: (h) => connection.listen(e => h(e))
				};
	...
	stop() {
		return this.hooks.run('stop', this, true);
	}
